"""
Clinical summary template endpoints.
"""
...
router = APIRouter(prefix="/clinical", tags=["Clinical Templates"])
...
async def create_clinical_template(
    ...
    template = await crud.create_template(db, TemplateType.CLINICAL, data, current_user.user_id)
    return template
...
async def list_clinical_templates(
    ...
    """List all clinical summary templates."""
...
async def get_clinical_template(
    ...
    """Get a specific clinical template."""
...
async def delete_clinical_template(
    template_id: str,
    db: AsyncSession = Depends(get_db),
    current_user: UserContext = Depends(get_current_user),
):
...
async def render_clinical_template(
    template_id: str,
    data: TemplateRenderRequest,
    db: AsyncSession = Depends(get_db),
    current_user: UserContext = Depends(get_current_user),
):
    """Render a clinical template to output format."""
