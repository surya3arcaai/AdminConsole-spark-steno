...
import { Plus, X, User, Edit2, Trash2, Key, ChevronRight, ChevronLeft, Check, FileText, Mic, ExternalLink, Shield } from "lucide-react";

const WIZARD_STORAGE_KEY = "arca_user_wizard_state";

export default function UserManager({ initialUsers = [], demographics = {}, templates = {} }) {
    ...
    useEffect(() => {
        try {
            ...
            if (saved) {
                ...
                setWizardStep(parsed.wizardStep || 2);
                setShowEditor(true);
    ...
    const openViewer = async (user) => {
        setViewingUser({ ...user, registration_number: "Loading...", council_name: "Loading...", supervisor_id: null });
        try {
            const regData = await getRegistrationAction(user.id);
            const supData = await getSupervisorAction(user.id);
            setViewingUser(prev => ({
    ...
    const renderStep3 = () => (
        <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
            <div style={{ background: "rgba(0,0,0,0.2)", padding: "2rem", borderRadius: "12px", border: "1px solid var(--panel-border)", textAlign: "center" }}>
                <div style={{ width: "80px", height: "80px", borderRadius: "50%", background: "rgba(102, 252, 241, 0.1)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 1.5rem" }}>
                    <Mic size={36} color="var(--primary-color)" />
                </div>
                <h4 style={{ margin: "0 0 0.75rem 0", color: "var(--primary-color)" }}>Audio Samples</h4>
                <p style={{ color: "var(--text-secondary)", fontSize: "0.9rem", maxWidth: "450px", margin: "0 auto 1.5rem" }}>
                    Record or upload audio samples for voice recognition calibration. This helps Arca Spark Steno accurately transcribe this user&apos;s dictation.
                </p>

                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "1rem", maxWidth: "500px", margin: "0 auto" }}>
                    <div style={{ background: "rgba(0,0,0,0.2)", padding: "1.25rem", borderRadius: "10px", border: "1px dashed var(--panel-border)" }}>
                        <Mic size={24} color="var(--text-secondary)" style={{ margin: "0 auto 0.5rem", display: "block" }} />
                        <div style={{ fontSize: "0.85rem", color: "var(--text-secondary)" }}>Sample 1</div>
                        <div style={{ fontSize: "0.75rem", color: "var(--text-secondary)", marginTop: "0.25rem" }}>Not recorded</div>
                    </div>
                    <div style={{ background: "rgba(0,0,0,0.2)", padding: "1.25rem", borderRadius: "10px", border: "1px dashed var(--panel-border)" }}>
                        <Mic size={24} color="var(--text-secondary)" style={{ margin: "0 auto 0.5rem", display: "block" }} />
                        <div style={{ fontSize: "0.85rem", color: "var(--text-secondary)" }}>Sample 2</div>
                        <div style={{ fontSize: "0.75rem", color: "var(--text-secondary)", marginTop: "0.25rem" }}>Not recorded</div>
                    </div>
                    <div style={{ background: "rgba(0,0,0,0.2)", padding: "1.25rem", borderRadius: "10px", border: "1px dashed var(--panel-border)" }}>
                        <Mic size={24} color="var(--text-secondary)" style={{ margin: "0 auto 0.5rem", display: "block" }} />
                        <div style={{ fontSize: "0.85rem", color: "var(--text-secondary)" }}>Sample 3</div>
                        <div style={{ fontSize: "0.75rem", color: "var(--text-secondary)", marginTop: "0.25rem" }}>Not recorded</div>
                    </div>
                </div>

                <p style={{ color: "var(--text-secondary)", fontSize: "0.8rem", marginTop: "1.5rem", fontStyle: "italic" }}>
                    Audio sample recording will be available in a future update. You can skip this step for now.
                </p>
            </div>
                ...
                <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.6)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <div className="glass-panel slide-up" style={{ width: "450px", padding: "2rem" }}>
                        ...
                        <div style={{ display: "flex", gap: "1rem", marginTop: "1.5rem" }}>
                            <button className="btn-primary" onClick={handleSubSubmit} style={{ flex: 1 }}>Save To Database</button>
                            <button className="btn-secondary" onClick={() => setAddingType(null)} style={{ flex: 1 }}>Cancel</button>
                        </div>
                    </div>
                </div>
            )}
                ...
                <div className="glass-panel slide-up" style={{ animationDelay: "0.1s", display: "flex", flexDirection: "column", gap: "1.5rem" }}>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <h3 style={{ margin: 0, display: "flex", alignItems: "center", gap: "0.5rem" }}>
                            ...
                            {form.id ? "Edit User Profile" : "New User Onboarding"}
                        </h3>
                        <button onClick={() => { setShowEditor(false); setWizardStep(1); }} style={{ color: "var(--text-secondary)", background: "none", cursor: "pointer", border: "none" }}>
                            <X size={24} />
                        </button>
                    </div>
                    ...
                    {/* Step Content */}
                    {(form.id || wizardStep === 1) && renderStep1()}
                    {!form.id && wizardStep === 2 && renderStep2()}
                    {!form.id && wizardStep === 3 && renderStep3()}

                    {/* Navigation Footer */}
                    <div style={{ display: "flex", gap: "1rem", marginTop: "1rem", justifyContent: "space-between" }}>
                        <div>
                            {!form.id && wizardStep > 1 && (
                                <button className="btn-secondary" onClick={handlePrevStep} style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                                    <ChevronLeft size={16} /> Back
                                </button>
                            )}
                        </div>
                        <div style={{ display: "flex", gap: "1rem" }}>
                            <button className="btn-secondary" onClick={() => { setShowEditor(false); setWizardStep(1); }}>Cancel</button>
                            {form.id ? (
                                <button className="btn-primary" onClick={handleFinalSave}>Update User</button>
                            ) : wizardStep < 3 ? (
                                <button className="btn-primary" onClick={handleNextStep} style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                                    Next <ChevronRight size={16} />
                                </button>
                            ) : (
                                <button className="btn-primary" onClick={handleFinalSave} style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                                    <Check size={16} /> Create User & Save
                                </button>
                            )}
                        </div>
                    </div>
                                    ...
                                    <tr key={u.id || i} className="clickable-row" style={{ borderBottom: "1px solid var(--panel-border)", transition: "background 0.2s ease" }}>
                                        ...
                                        <td style={{ padding: "1rem 1.5rem", textAlign: "right" }}>
                                            <div style={{ display: "flex", justifyContent: "flex-end", gap: "0.5rem" }}>
                                                <button onClick={() => openEditor(u)} style={{ color: "var(--primary-color)", background: "rgba(102, 252, 241, 0.1)", cursor: "pointer", padding: "0.5rem", borderRadius: "8px", border: "none", display: "flex" }}>
                                                    <Edit2 size={16} />
                                                </button>
                                                <button onClick={() => handleDelete(u.id)} style={{ color: "var(--error)", background: "rgba(255, 82, 82, 0.1)", cursor: "pointer", padding: "0.5rem", borderRadius: "8px", border: "none", display: "flex" }}>
                                                    <Trash2 size={16} />
                                                </button>
                                            </div>
                                        </td>
